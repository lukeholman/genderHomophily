---
title: "Homophily in science: do researchers preferentially collaborate with same-gendered colleagues?"
...
output:
  html_document: 
    ...
    theme: lumen # Luke's favourite fonts and colours
...
library(kableExtra) # scrollable table
...
library(Cairo) # for exporting the alpha characters via ggsave
...
library(knitr) # set up cacheing to save time when re-building the html document - chunks are not re-evaluated if the code has not changed
...
Importantly, this function discards all papers where we do not know the gender of every single author. This is conservative but greatly simplifies the test -- we'd have to be careful how we included papers where some author's genders were unknown. Our failures to identify author gender are almost certainly randomly distributed across papers with respect to gender homophily, this omission makes our tests less sensitive but should not bias their outcomes. 
...
convert.dates <- function(dates) {
  dates <- as.numeric(as.Date(dates, "%d_%m_%Y")) - as.numeric(as.Date("20_08_2016","%d_%m_%Y")) # Convert the date to days before the present
  return(dates / 365.25)  # Convert from days to years
}

retrieve.authorship.data <- function(journals, minimum.n, start.of.time.period, end.of.time.period, file.name, over.write = FALSE){
  
  if(over.write | !(file.name %in% list.files())){ # Only do the test if over.write = TRUE or there is no datafile in the working directory
    ...
    # Loop over all the journals, try to get the data, and if there is enough, store the data we need as a list entry in 'count.data'
    for(i in 1:length(journals)){
      ...
      # Genders were only recorded as M or F if the person's given name is associated with one gender >95% of the time.
      focal <- pubmed_sqlite %>% 
      ...
      # For papers from between 5 and 6 yeras ago, set start.of.time.period = 6 and end.of.time.period = 5
      focal <- focal %>% 
      ...
      if(nrow(focal) >= minimum.n){ 
        ...
        if(length(focal) >= minimum.n){ 
          ...
          rm(focal) # Explicitly discard 'focal', in case this helps to reduce memory footprint
...
# 2-author vs 10-author papers have different amounts of homophilly, or 
...
gender.grouping.test.many.journals <- function(count.data, authors, output.file, nBoots, nSimulations, number.of.cores, over.write = FALSE){
  ...
  if(over.write | !(output.file %in% list.files())){ 
...
make.sample.size.table <- function(filename){
  ...
  count.data <- readRDS(paste(filename, ".rds", sep = "")) 
...
make.sample.size.table("older.papers") %>% 
...
gender.grouping.test.many.journals(readRDS("last.year.rds"),
                                   "all",
                                   "homophily.results.all.csv",
                                   nBoots = 1000, 
                                   nSimulations = 1000, 
                                   number.of.cores = 8,
                                   over.write = FALSE)
...
gender.grouping.test.many.journals(readRDS("last.year.rds"),
                                   "first",
                                   "homophily.results.first.authors.csv",
                                   nBoots = 1000, 
                                   nSimulations = 1000, 
                                   number.of.cores = 8,
                                   over.write = FALSE)
...
gender.grouping.test.many.journals(readRDS("last.year.rds"),
                                   "last",
                                   "homophily.results.last.authors.csv",
                                   nBoots = 1000, 
                                   nSimulations = 1000, 
                                   number.of.cores = 8,
                                   over.write = FALSE)
...
gender.grouping.test.many.journals(readRDS("older.papers.rds"),
                                   "all",
                                   "older.homophily.results.all.csv",
                                   nBoots = 1000, 
                                   nSimulations = 1000, 
                                   number.of.cores = 8,
                                   over.write = FALSE)
...
gender.grouping.test.many.journals(readRDS("older.papers.rds"),
                                   "first",
                                   "older.homophily.results.first.authors.csv",
                                   nBoots = 1000, 
                                   nSimulations = 1000, 
                                   number.of.cores = 8,
                                   over.write = FALSE)
...
gender.grouping.test.many.journals(readRDS("older.papers.rds"),
                                   "last",
                                   "older.homophily.results.last.authors.csv",
                                   nBoots = 1000, 
                                   nSimulations = 1000, 
                                   number.of.cores = 8,
                                   over.write = FALSE)
...
Here, we assign extra information to each journal - research discipline (see Holman _et al._ in prep), ISI impact factor (where available), and the country that publishes the journal (where known; information taken from PubMed).
...
merge.and.clean <- function(homophily.filepath, journal.data){
  left_join(read.csv(homophily.filepath, stringsAsFactors = FALSE), journal.data, by = "journal") %>% 
    select(discipline, 
           journal, 
           standardised.IF, 
           country,
...
gender.homophily.all <- merge.and.clean("homophily.results.all.csv", journal.data)
gender.homophily.first <- merge.and.clean("homophily.results.first.authors.csv", journal.data)
gender.homophily.last <- merge.and.clean("homophily.results.last.authors.csv", journal.data)

gender.homophily.all.old <- merge.and.clean("older.homophily.results.all.csv", journal.data)
gender.homophily.first.old <- merge.and.clean("older.homophily.results.first.authors.csv", journal.data)
gender.homophily.last.old <- merge.and.clean("older.homophily.results.last.authors.csv", journal.data)
...
write.csv(neat, file = "Supplementary dataset S1.csv", row.names = FALSE)
rm(neat)
```
...
if(!("country.data.csv" %in% list.files())){ # Write the data, assuming it's not already been done
  country.data <- gender.grouping.test.by.journal.and.country(country.data)
  write.csv(country.data, file = "country.data.csv", row.names = FALSE)
}

country.data <- read.csv("country.data.csv", stringsAsFactors = FALSE) %>%
...
master.dataset %>%
  filter(Age == "New") %>%
  ggplot(aes(x = log10(n.useable.papers), y = alpha.adjusted - observed.alpha)) + 
  geom_point(alpha = 0.3, size = 0.5) + 
  facet_wrap(~Subset) +
  ylab("\u03B1' - \u03B1") + xlab("Log10 sample size\n(number of papers)") + 
  theme_minimal()
```
<br></br> 
...
n.sig.homo <- sum(figure2$Significant[figure2$Subset == "All authors"] == "Yes" & 
                    figure2$alpha.adjusted[figure2$Subset == "All authors"] > 0)
n.sig.hetero <- sum(figure2$Significant[figure2$Subset == "All authors"] == "Yes" & 
                    figure2$alpha.adjusted[figure2$Subset == "All authors"] < 0)
n.journals <- sum(figure2$Subset == "All authors")
...
ggsave(plot = figure2, filename = "figures/figure2.pdf",  width = 5, height = 8, device = cairo_pdf)
figure2
```
<br></br> 
...
```
<br></br>
**Figure S`r supp.figure.counter`**: Histogram showing the distribution of differences in $\alpha'$ between the 2015-16 and 2005-6 samples, where positive numbers indicate an increase in $\alpha'$ with time. The mean is slightly greater than 0, indicating a mild increase in average $\alpha'$ with time.  
...
```{r}
summary(lmer(scale(alpha.adjusted) ~ Age + (1|journal) + (1|discipline), # Note that the response variable gets scaled, to make the effect size interpretable as Cohen's d
             weights = 1 / ((upper.95.CI2 - lower.95.CI2)/2*1.96), # Weight each observation of alpha by its precision
             data = master.dataset %>% 
...
gender.homophily.all %>% 
  ...
  ggplot(aes(x = Discipline, y = alpha.adjusted, fill = Discipline)) +
...
```
<br></br>
**Figure `r figure.counter`**: Distribution of the $\alpha'$ values across journals, split by the research discipline of the journal. The points show the $\alpha'$ for each journal, and the boxes show quartiles 1, 2 and 3. The boxplot whiskers show the distance of the farthest point that is within 1.5 times the interquartile range from the edge of the box.
...
**Table S`r supp.table.counter`**: The table shows similar information to Figure S`r supp.figure.counter`, namely the distribution of the $\alpha'$ values across journals, split by the research discipline. The gender ratio column shows the percentage of women authors in the sample used to calculate $\alpha'$, across all authorship positions. In the last two columns, the numbers outside parentheses give the number of journals that deviate statistically significantly from zero, while the numbers inside parentheses give the number that remain significant after false discovery rate correction.
...
gender.ratio <- master.dataset %>% 
  ...
  stat_smooth(method = "gam", formula = y ~ s(x, k = 3), colour = "black", fill = "skyblue") + 
  xlab("Deviation from gender parity for all authors in this journal") + 
  ylab("Coefficient of homophily (\u03B1')") +
  facet_wrap(~Subset) + 
  theme_minimal() +
  theme(legend.position = "none") 
ggsave(plot = gender.ratio, filename = "figures/figure3.pdf", device = cairo_pdf, width=7, height=5)
gender.ratio
```
<br></br>
...
gender.homophily.all %>% 
  ...
  geom_hline(yintercept = 0, linetype = 2) +
  geom_vline(xintercept = 0, linetype = 2) +
  stat_smooth(method = "lm", colour = "black", fill = "skyblue") +
...
### Gender homophilly is also observed within countries
...
country.histo <- country.data %>%
  ...
  theme_minimal() + 
  theme(legend.position = "top")
ggsave(plot = country.histo, filename = "figures/figure4.pdf", device = cairo_pdf, width = 6, height = 5)
country.histo
```
<br></br>
