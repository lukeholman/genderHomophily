...
sample.size.table %>% pander(split.cell = 40, split.table = Inf)
```
...
gender.ratio <- master.dataset %>% 
  ...
  stat_smooth(method = "gam", formula = y ~ s(x, k = 3), colour = "tomato", fill = "skyblue") + 
...
simulate <- function(params){
  ...
  # given the gender ratio at career stages a and b, find the proportion of collaborations that are man-man, woman-woman, or mixed gender
  ...
  # Find p and q, which are used to calcualte alpha
...
simulation.figure <- results %>%
  ggplot(aes(GR.stage1, GR.stage2, z = alpha, fill = alpha)) + 
  facet_grid(prop.student.prof ~ prop.student.student) + 
  geom_tile() + 
  #stat_contour(binwidth = 0.1, colour = "grey20", size = 0.3) + 
